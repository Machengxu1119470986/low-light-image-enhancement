...
def mutual_i_loss(input_I_low, input_I_high):
    ...
    x_loss = (low_gradient_x + high_gradient_x)* tf.exp(-10*(low_gradient_x+high_gradient_x))#公式2.2.1
    ...
    mutual_loss = tf.reduce_mean( x_loss + y_loss) #loss公式
    return mutual_loss
...
def mutual_i_input_loss(input_I_low, input_im):
    ...
    return mut_loss
#分解出的两个图相乘 与 原来的图应该一样（Retinex理论）保证Decom的正确性
