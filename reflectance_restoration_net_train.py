...
config = tf.ConfigProto()#配置session运算方式，看使用GPU或是CPU运算
config.gpu_options.allow_growth = True
sess=tf.Session(config=config)
#the input of decomposition net
input_decom = tf.placeholder(tf.float32, [None, None, None, 3], name='input_decom')

input_ss = tf.placeholder(tf.float32, [None, None, None, 3], name='input_ss')
#restoration input
...
#核心修改地方
output_r = Restoration_net(input_low_r, input_low_i, input_ss)
...
def ssim_loss(output_r, input_high_r):
    output_r_1 = output_r[:,:,:,0:1]#x[:,m:n]，即取所有数据集的第m到n-1列数据
...
update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)#tf.get_collection会列出key里所有的值，key为update_ops的集合
#tf.GraphKeys.UPDATE_OPS，这是一个tensorflow的计算图中内置的一个集合，其中会保存一些需要在训练操作之前完成的操作，并配合tf.control_dependencies函数使用。
optimizer = tf.train.AdamOptimizer(learning_rate=lr, name='AdamOptimizer')
#损失函数优化器的minimize()中global_step=global_step能够提供global_step自动加一的操作
#指数衰减的学习率是伴随global_step的变化而衰减的，所以当global_step不改变时，学习率也变成一个定值
#global_step=global_step去掉，global_step的自动加一就会失效
...
var_Decom = [var for var in tf.trainable_variables() if 'DecomNet' in var.name]#tf.trainable_variables () 指的是需要训练的变量
...
for idx in range(len(train_low_data)):
    ...
    RR0 = np.squeeze(RR)  # 从数组的形状中删除单维条目，即把shape中为1的维度去掉
    II0 = np.squeeze(II)
...
#np.expand_dims(a, axis=0)表示在0位置添加数据，np.expand_dims:用于扩展数组的形状
for idx in range(len(eval_low_data)):
    ...
    RR03 = np.squeeze(RR3)    # 从数组的形状中删除单维条目，即把shape中为1的维度去掉
    II03 = np.squeeze(II3)
...
#eval_s_data = train_low_data[467:480] + eval_low_data[0:15]
eval_s = train_low_data[467:480] + eval_low_data[0:15]
...
train_s_data = train_low_data[0:466]
...
for epoch in range(start_epoch, epoch):
    for batch_id in range(start_step, numBatch):
        ...
        batch_input_s = np.zeros((batch_size, patch_size, patch_size, 3), dtype="float32")
        for patch_id in range(batch_size):
            ...
            batch_input_s[patch_id, :, :, :] = data_augmentation(train_s_data[image_id][x : x+patch_size, y : y+patch_size, :], rand_mode)
        ...
        _, loss = sess.run([train_op, train_loss], feed_dict={input_low_r: batch_input_low_r,input_low_i: batch_input_low_i,\
                                                              input_high_r: batch_input_high_r,input_ss:batch_input_s, lr: lr_schedule(epoch)})
    ...
    if (epoch + 1) % eval_every_epoch == 0:
        ...
        for idx in range(len(eval_restoration_low_r_data)):
            ...
            input_all_s = eval_s[idx] 
            eval_s_data = np.expand_dims(input_all_s, axis=0)
            
            result_1 = sess.run(output_r, feed_dict={input_low_r: input_low_eval_r, input_low_i: input_low_eval_ii,input_ss:eval_s_data})
