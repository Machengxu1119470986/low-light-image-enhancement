...
for idx in range(len(train_low_data)):
    ...
    decomposed_low_i_data_480.append(II0)#与第二个网络有些许不同
for idx in range(len(train_high_data)):
    ...
    RR02 = np.squeeze(RR2)#与第二个网络有些许不同
    II02 = np.squeeze(II2)
    print(RR02.shape, II02.shape)
    #decomposed_high_r_data_480.append(result_1_sq)
    decomposed_high_i_data_480.append(II02)#这里的i和r不同，与第二个网络相比，主要是看loss流程图里面涉及的是什么

eval_adjust_low_i_data = decomposed_low_i_data_480[451:480]#数据范围不同
...
for epoch in range(start_epoch, epoch):
    for batch_id in range(start_step, numBatch):
        batch_input_low_i_ratio = np.zeros((batch_size, patch_size, patch_size, 1), dtype="float32")#这里有出入
        ...
        for patch_id in range(batch_size):
            ...
            #i_low_data_ratio = np.ones([patch_size,patch_size])*(1/20+0.0001)
