...
#使用反卷积来进行上采样操作，不需要借助插值方法，可以提供能够学习的参数
...
def Restoration_net(input_r, input_i,input_s):
    with tf.variable_scope('Restoration_net', reuse=tf.AUTO_REUSE):
        input_xz = tf.concat([input_s,input_i],3)
        conv00=slim.conv2d(input_xz,32,[3,3], rate=1, activation_fn=lrelu,scope='de_conv0_0')
        conv01=slim.conv2d(conv00,32,[3,3], rate=1, activation_fn=lrelu,scope='de_conv0_1')
        conv02=slim.conv2d(conv01,32,[3,3], rate=1, activation_fn=lrelu,scope='de_conv0_2')
        conv03=slim.conv2d(conv02,32,[3,3], rate=1, activation_fn=lrelu,scope='de_conv0_3')
        conv04=slim.conv2d(conv03,32,[3,3], rate=1, activation_fn=lrelu,scope='de_conv0_4')
        conv05=slim.conv2d(conv04,1,[1,1], rate=1, activation_fn=None,scope='de_conv0_5')
        conv06=tf.sigmoid(conv05)
        input_cbam=input_r*conv06+input_r
        
        
        conv1=slim.conv2d(input_cbam,32,[3,3], rate=1, activation_fn=lrelu,scope='de_conv1_1')
